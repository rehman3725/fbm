...
_DB_HEALTH_SNAPSHOT_PATH = os.path.join(legacy_instance_dir, 'health_snapshot.json')
...
if not _root_logger.handlers:
    ...
    file_handler = logging.FileHandler('errorlog.txt', encoding='utf-8')
...
def _start_reconcile_worker():
    global _RECON_WORKER_STARTED
    if _RECON_WORKER_STARTED:
        return
...
ENDPOINT_PERMISSION_MAP = {
    ...
    'fbm_cash_drawer_page': 'can_manage_payments',
    'add_fbm_cash_drawer_entry': 'can_manage_payments',
    'edit_fbm_cash_drawer_entry': 'can_manage_payments',
    'void_fbm_cash_drawer_entry': 'can_manage_payments',
    'add_fbm_cash_drawer_category': 'can_manage_payments',
    'edit_fbm_cash_drawer_category': 'can_manage_payments',
    'toggle_fbm_cash_drawer_category': 'can_manage_payments',
...
@app.route('/fbm_cash_drawer')
@login_required
def fbm_cash_drawer_page():
    date_from_raw = (request.args.get('date_from') or '').strip()
    date_to_raw = (request.args.get('date_to') or '').strip()
    date_from = _drawer_parse_date(date_from_raw)
    date_to = _drawer_parse_date(date_to_raw)
    flow = (request.args.get('flow') or 'all').strip().lower()
    if flow not in ['all', 'in', 'out']:
        flow = 'all'
    source = (request.args.get('source') or 'all').strip().lower()
    if source not in ['all', 'manual', 'client_payments', 'material_sales', 'balance']:
        source = 'all'
    per_page = request.args.get('per_page', '50')
    try:
        per_page_int = int(per_page or 50)
    except Exception:
        per_page_int = 50
    if per_page_int not in [20, 50, 100, 200]:
        per_page_int = 50
    ...
    period_kpis = _drawer_kpis(date_from=date_from, date_to=date_to)
    today = pk_today()
    today_kpis = _drawer_kpis(date_from=today, date_to=today)
    rows = _drawer_unified_rows(
        date_from=date_from,
        date_to=date_to,
        source=source,
        flow=flow,
    ...
    if kpi == 'client_payments':
        source = 'client_payments'
        flow = 'in'
    elif kpi == 'material_sales':
        source = 'material_sales'
        flow = 'in'
    elif kpi == 'cash_spent_manual':
        source = 'manual'
        flow = 'out'
    elif kpi == 'drawer_balance':
        source = 'all'

    total_rows = len(rows)
    total_pages = max(1, (total_rows + per_page_int - 1) // per_page_int)
    if page > total_pages:
        page = total_pages
    ...
    category_rows = FbmCashDrawerCategory.query.order_by(FbmCashDrawerCategory.name.asc()).all()
    categories = [str(r.name or '').strip() for r in category_rows if str(r.name or '').strip()]
    active_categories = [
        str(r.name or '').strip()
        for r in category_rows
        if str(r.name or '').strip() and bool(getattr(r, 'is_active', True))
    ]
    for r in FbmCashDrawerEntry.query.filter(FbmCashDrawerEntry.is_void == False).all():
        c = str(r.category or '').strip()
        if c and c not in categories:
            categories.append(c)
    categories = sorted(categories, key=lambda x: x.lower())
    active_categories = sorted(list({x for x in active_categories}), key=lambda x: x.lower())

    return render_template(
        'fbm_cash_drawer.html',
        today_date=pk_today().strftime('%Y-%m-%d'),
        ...
        drawer_rows=page_rows,
    )


@app.route('/fbm_cash_drawer/add', methods=['POST'])
@login_required
def add_fbm_cash_drawer_entry():
    entry_type = (request.form.get('entry_type') or 'out').strip().lower()
    if entry_type not in ['in', 'out']:
        entry_type = 'out'

    method = (request.form.get('method') or 'Cash').strip()
    if method not in ['Cash', 'Bank', 'Check', 'Cheque']:
        method = 'Cash'
    ...
    if amount <= 0:
        ...
        return redirect(url_for('fbm_cash_drawer_page'))

    category = (request.form.get('category') or '').strip()
    if category:
        _drawer_upsert_category(category)
    note = (request.form.get('note') or '').strip()
    date_str = (request.form.get('date') or '').strip()
    posted_at = resolve_posted_datetime(date_str)

    row = FbmCashDrawerEntry(
    ...
    if _drawer_is_cash_method(method):
        flash('Drawer entry saved.', 'success')
    else:
        flash('Entry saved as non-cash record. It will not affect FBM cash drawer totals.', 'info')
    return redirect(url_for('fbm_cash_drawer_page'))


@app.route('/fbm_cash_drawer/edit/<int:entry_id>', methods=['POST'])
@login_required
def edit_fbm_cash_drawer_entry(entry_id):
    row = FbmCashDrawerEntry.query.get_or_404(entry_id)
    if (row.source or 'manual').strip().lower() != 'manual':
        flash('Only manual drawer entries are editable.', 'danger')
        return redirect(url_for('fbm_cash_drawer_page'))
    entry_type = (request.form.get('entry_type') or row.entry_type or 'out').strip().lower()
    if entry_type not in ['in', 'out']:
        entry_type = 'out'
    method = (request.form.get('method') or row.method or 'Cash').strip()
    if method not in ['Cash', 'Bank', 'Check', 'Cheque']:
        method = 'Cash'
    try:
        amount = float(request.form.get('amount', row.amount or 0) or 0)
    except Exception:
        amount = 0
    if amount <= 0:
        flash('Amount must be greater than zero.', 'danger')
    ...
    if category:
        _drawer_upsert_category(category)
    note = (request.form.get('note') or '').strip()
    posted_at = resolve_posted_datetime((request.form.get('date') or '').strip(), fallback_dt=row.date_posted or pk_now())
    ...
    row.note = note
    row.date_posted = posted_at
    db.session.commit()
    flash('Drawer entry updated.', 'success')
    return redirect(url_for('fbm_cash_drawer_page'))


@app.route('/fbm_cash_drawer/category/add', methods=['POST'])
@login_required
def add_fbm_cash_drawer_category():
    name = (request.form.get('name') or '').strip()
    if not name:
        flash('Category name is required.', 'danger')
        return redirect(url_for('fbm_cash_drawer_page'))
    _drawer_upsert_category(name)
    db.session.commit()
    flash('Drawer category saved.', 'success')
    return redirect(url_for('fbm_cash_drawer_page'))


@app.route('/fbm_cash_drawer/category/edit/<int:category_id>', methods=['POST'])
@login_required
def edit_fbm_cash_drawer_category(category_id):
    row = FbmCashDrawerCategory.query.get_or_404(category_id)
    name = (request.form.get('name') or '').strip()
    if not name:
        flash('Category name is required.', 'danger')
        return redirect(url_for('fbm_cash_drawer_page'))
    dup = FbmCashDrawerCategory.query.filter(
        FbmCashDrawerCategory.id != row.id,
...
@app.route('/fbm_cash_drawer/category/toggle/<int:category_id>', methods=['POST'])
@login_required
def toggle_fbm_cash_drawer_category(category_id):
    row = FbmCashDrawerCategory.query.get_or_404(category_id)
    row.is_active = not bool(getattr(row, 'is_active', True))
    db.session.commit()
    flash(f"Category {'activated' if row.is_active else 'suspended'}.", 'success')
    return redirect(url_for('fbm_cash_drawer_page'))


@app.route('/fbm_cash_drawer/void/<int:entry_id>', methods=['POST'])
@login_required
def void_fbm_cash_drawer_entry(entry_id):
    row = FbmCashDrawerEntry.query.get_or_404(entry_id)
    if (row.source or 'manual').strip().lower() != 'manual':
        flash('Only manual drawer entries can be voided.', 'danger')
        return redirect(url_for('fbm_cash_drawer_page'))
    row.is_void = True
    db.session.commit()
    flash('Drawer entry voided.', 'success')
    return redirect(url_for('fbm_cash_drawer_page'))
...
@login_required
def add_payment():
    flash('Customer payment posting is disabled. Use Accounts → New Transaction (Receive).', 'danger')
...
@login_required
def edit_payment(id):
    flash('Customer payment editing is disabled. Use Accounts → Audit / Ledger actions instead.', 'danger')
    return redirect(url_for('accounts.dashboard'))
...
def direct_sales_page():
    ...
    for client_norm, mat_name, qty in booked_rows_all:
        ...
        mat_map[mat_name] = float(qty or 0)
    ...
    for client_code, client_norm, mat_name, qty in delivered_rows_all:
        ...
        if client_code:
            ...
            mat_map_code[mat_name] = float(qty or 0)
...
@app.route('/direct_sales/hold', methods=['POST'])
@login_required
...
@app.route('/direct_sales/hold/<int:draft_id>/resume', methods=['POST'])
@login_required
...
@app.route('/direct_sales/hold/<int:draft_id>/delete', methods=['POST'])
@login_required
...
def add_direct_sale():
  try:
    ...
    for idx, mat in enumerate(materials_list):
        ...
        balance = 0 if ignore_item else booking_balances.get(mat_name, 0)
        qty_booking = 0
        qty_sale = qty

        if category == 'Credit Customer' and not ignore_item and balance > 0:
            return _fail_sale('Credit Sale cannot use booked balance. Check Ignore for booked items to sell as normal.')
...
@app.route('/add_material_return', methods=['POST'])
@login_required
...
@app.route('/edit_material_return/<int:id>', methods=['POST'])
@login_required
def edit_material_return(id):
    ...
    try:
        ...
        if not ret:
            ...
            return redirect(url_for('material_returns_page'))
        if ret.is_void:
            flash('Voided return cannot be edited.', 'danger')
            return redirect(url_for('material_returns_page'))
        ...
        if new_type != existing_type:
            flash('Return type cannot be changed on edit. Void and create a new return.', 'danger')
...
def void_transaction(type, id):
    ...
    if type == 'Entry':
        ...
        if _set_material_return_void_state(ret, True):
            flash('Material return voided', 'success')
...
def delete_bill(type, id):
    ...
    if type == 'Booking':
        ...
        bill = db.session.get(MaterialReturn, id)
        if bill:
            changed = _set_material_return_void_state(bill, True)
...
def financial_ledger(client_id):
    ...
    deliveries = Entry.query.filter(
        ...
        or_(
            ...
            and_(Entry.type == 'IN', Entry.nimbus_no == 'Material Return')
        )
...
def profit_reports():
    ...
    for item, sale in direct_rows:
        ...
        # Check if this sale item has a specific GRN linked
        cost_rate, cogs_known = _cost_rate_for_grn_item(item.grn_item_id)
        if not cogs_known:
            # Fall back to general material cost lookup
...
@app.route('/delivery_rents/pay/<int:alloc_id>', methods=['POST'])
@login_required
...
def merge_materials():
    ...
    try:
        ...
        # 4. Update MaterialReturnItem records
        MaterialReturnItem.query.filter_by(material_name=source_name).update({'material_name': target_name})

        # 5. Update GRNItem records
        GRNItem.query.filter_by(mat_name=source_name).update({'mat_name': target_name})

        # 6. Update DeliveryItem records
    ...
    if request.method == 'POST':
        ...
        if action == 'add':
            ...
            if grn_obj:
                ...
                if restricted:
                    return restricted
                _set_grn_void_state(grn_obj, True)
                db.session.commit()
                flash('GRN voided successfully!', 'success')
...
def delete_supplier_payment(id):
    flash('Supplier payment voiding is disabled. Use Accounts → Audit / Ledger actions instead.', 'danger')
    return redirect(url_for('accounts.dashboard'))
    payment = SupplierPayment.query.get_or_404(id)
...
# ==================== CENTRALIZED PAY-SUPPLIER PAGE ====================

@app.route('/pay_supplier', methods=['GET'])
@login_required
def pay_supplier_page():
    flash('Pay Supplier workflow is disabled. Use Accounts → New Transaction (Pay → Supplier).', 'danger')
    return redirect(url_for('accounts.dashboard'))
    """Single-screen workflow: pick supplier, see live payable, pay from one of our accounts."""
    suppliers_q = Supplier.query.filter(
        func.coalesce(Supplier.is_active, True) == True
    ).order_by(Supplier.name.asc()).all()

    suppliers_with_balance = []
    for s in suppliers_q:
        try:
            _rows, bal, total_bill, total_paid = _build_supplier_ledger_rows(s)
        except Exception:
            bal, total_bill, total_paid = (s.opening_balance or 0), 0, 0
    ...
    today_date = pk_now().strftime('%Y-%m-%d')
    return render_template(
        'pay_supplier.html',
...
# ==================== BLUEPRINTS ====================
# Blueprints are now loaded automatically by load_modules() above
